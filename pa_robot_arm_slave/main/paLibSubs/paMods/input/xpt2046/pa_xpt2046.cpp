...
pa_Xpt2046::pa_Xpt2046() {}

uint16_t pa_Xpt2046::spiRead()
{
	uint8_t i = 12;
	uint16_t value = 0;
...
void pa_Xpt2046::spiWrite(uint8_t value)
{
	uint8_t i = 0x08;
...
uint8_t pa_Xpt2046::isPressed()
{
	return !Hardware_ReadIRQ();
}

uint8_t pa_Xpt2046::readRaw(uint16_t Coordinates[2])
{
...
void pa_Xpt2046::turnRawToScreen(uint16_t Coordinates[2])
{
	int x = Coordinates[0];
	int y = Coordinates[1];
...
void pa_Xpt2046::init(
	int screenW,
	int screenH,
	int xBeginRaw,//根据raw信息自动在区间范围插值
	int xEndRaw,
	int yBeginRaw,
